...
describe('PDF', () => {
  ...
  it('Verify Pdf downloaded from an archived reporting service', () => {
    ...
    cy.get(partySelect.reporteeSelf)
      .find(partySelect.reporteeButton)
      .should('be.visible')
      .click()
    cy.get(common.archives).click()
    cy.get('.row.a-inboxHeadingContent').click()
    cy.get('div[id*="accordionItem-messageBox"]')
      .find('button[onclick*="RedirectToArchivedElement"]')
      .click()
    cy.get('a[id*=ReceiptPrintFormSet]').click()
    cy.readFile(
      `${Cypress.config('downloadsFolder')}/${texts.accSecurityLevel2Mag}.pdf`,
      'binary',
    ).should('exist')
    cy.task('getPdfContent', `${texts.accSecurityLevel2Mag}.pdf`).then(
      (content) => {
